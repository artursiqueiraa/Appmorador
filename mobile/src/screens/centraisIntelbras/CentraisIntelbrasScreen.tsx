...
type CentraisIntelbrasRouteProp = RouteProp<RootStackParamList, 'CentraisIntelbras'>;

/**
 * Sprint 15 — Integração Intelbras: Prova Definitiva da Arquitetura (ADR 0018).
 * Mesmo Equipamento genérico já usado por Control iD/JFL (Fabricante fixo em
 * "Intelbras"), formulário com Ip/Porta/Senha (a central disca para o AppMorador
 * receber comandos locais — não tem conceito de usuário, só senha de acesso remoto).
 */
export function CentraisIntelbrasScreen() {
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();
  const { params } = useRoute<CentraisIntelbrasRouteProp>();
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      setCentrais(lista.filter((e) => e.fabricante === 'Intelbras'));
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Não foi possível carregar as centrais Intelbras.');
    } finally {
      setLoading(false);
    }
  }, [propriedadeId]);
  ...
  const abrirEdicao = (central: EquipamentoResponse) => {
    ...
    setIp(central.ip ?? '');
    setPorta(central.porta ? String(central.porta) : '');
    setSenha('');
    setShowForm(true);
  };

  const formValido = !!nome.trim() && !!ip.trim() && !!porta.trim() && (!!editando || !!senha.trim());
  ...
  const salvar = async () => {
    ...
    try {
      const payloadBase = {
        ...
        fabricante: 'Intelbras' as const,
        ip: ip.trim(),
        porta: Number(porta),
      };

      if (editando) {
        const atualizada = await api.put<EquipamentoResponse>(`/api/equipamentos/${editando.id}`, {
          ...payloadBase,
          senha: senha.trim() || undefined,
        });
        setCentrais((prev) => prev.map((c) => (c.id === atualizada.id ? atualizada : c)));
      } else {
        const criada = await api.post<EquipamentoResponse>(`/api/properties/${propriedadeId}/equipamentos`, {
          ...payloadBase,
          senha: senha.trim(),
        });
      ...
      setError(err instanceof ApiError ? err.message : 'Não foi possível salvar a central Intelbras.');
    } finally {
      setSalvando(false);
    }
  };
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerTextWrap}>
          <Text style={styles.title}>Centrais Intelbras</Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          onRefresh={carregar}
          renderItem={({ item }) => (
            <Pressable style={styles.card} onPress={() => navigation.navigate('DetalhesCentralIntelbras', { equipamentoId: item.id })}>
              <View style={styles.cardMain}>
                ...
                <View style={styles.cardTextWrap}>
                  ...
                  <Text style={styles.cardSubtitle}>{item.ip}:{item.porta}</Text>
                </View>
      ...
      {showForm ? (
        <View style={styles.form}>
          ...
          <TextField label="Modelo (opcional)" value={modelo} onChangeText={setModelo} placeholder="Ex.: AMT 8000" />
          <TextField label="IP" value={ip} onChangeText={setIp} placeholder="Ex.: 192.168.1.60" autoCapitalize="none" />
          <TextField label="Porta" value={porta} onChangeText={setPorta} placeholder="Ex.: 9600" keyboardType="number-pad" />
          <TextField
