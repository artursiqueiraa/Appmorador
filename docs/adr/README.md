...
| [0029](0029-endpoints-leitura-global-painel-web.md) | Endpoints de Leitura Global para o Painel Web: GET /api/proprietarios + GET /api/dashboard-operacional (Master/Suporte-only, só-leitura), Sessões Ativas sem revogação real de token | 2026-07-27 | Decidido |
| [0030](0030-arquitetura-painel-web.md) | Arquitetura do Painel Web: React 19 + Vite + TS, Zustand substitui contexts/, Vitest substitui Jest, impersonation via troca de token (sem header customizado), MUI v9 exige sx | 2026-07-28 | Decidido |
| [0031](0031-equipamentos-provisionamentos-diagnostico-painel-web.md) | Equipamentos, Provisionamentos e Diagnóstico no Painel Web: rotas `api/painel/...` isoladas do Mobile, `VinculoEquipamentoPropriedade` como entidade nova (não reaproveita `Provisionamento` ADR 0028), `EstadoOperacionalEquipamento` paralelo a `StatusEquipamento`, Diagnóstico estritamente leitura | 2026-07-29 | Decidido |
| [0033](0033-plataforma-execucao-comandos-hardware.md) | Plataforma de Execução de Comandos de Hardware (base para Sprint 22C, ainda não iniciada): `ProviderType`/`HardwareCommandType` como identificadores estáveis, Command Registry (capability→payload→converter→handler), idempotência com janela explícita, `ExecutionPipeline` com curto-circuito formalizado, `HardwareEvent` registrado como nota de arquitetura futura (não decidido); Rev. 3 acrescenta os 5 ajustes obrigatórios de prontidão de execução da Sprint 22C (§16) | 2026-07-29 | Aprovado (Rev. 3) |
