...
public sealed class ArmCommandResult
{
    ...
    /// <summary>Status completo devolvido pela central junto da resposta — reaproveitado pelo chamador para atualizar o snapshot de Dashboard sem uma segunda consulta.</summary>
    public CentralStatusResponse? StatusResultante { get; init; }

    public static ArmCommandResult Ok(bool estadoConfirmado, CentralStatusResponse statusResultante) =>
        new() { Sucesso = true, EstadoConfirmado = estadoConfirmado, StatusResultante = statusResultante };
...
/// padrao ja homologado por <see cref="PgmCommandService"/>. Portado de
/// Integra-o-FL-main/SDK/CentralHub.SDK (Sprint 12 — ADR 0014/0015).
/// </summary>
/// <remarks>
...
/// fosse uma particao (confirmado por captura real do legado: "COMANDO DE ARMAR O
/// ELETRIFICADOR", <c>7B 06 03 4E 63 53</c>) — nao e um bug de validacao, e intencional. O
/// eletrificador nao tem um modo "Stay" documentado, entao <see cref="ArmarStayAsync"/> nao
/// aceita particao 99.
/// </remarks>
public sealed class ArmCommandService
{
    ...
    /// normal — o protocolo nao documenta um estado "ArmadaAway" separado.
    /// </summary>
        ...
        CancellationToken cancellationToken, TimeSpan? timeout)
    {
        ...
        try
        {
            ...
            return confirmado
                ? ArmCommandResult.Ok(estadoEsperadoArmado, status)
