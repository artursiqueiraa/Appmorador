...
public sealed class ZoneInhibitResult
{
    ...
    public static ZoneInhibitResult Ok(CentralStatusResponse statusResultante) =>
        new() { Sucesso = true, ZonasResultantes = statusResultante.Zonas, StatusResultante = statusResultante };
...
/// Portado de Integra-o-FL-main/SDK/CentralHub.SDK (Sprint 12 — ADR 0014/0015).
/// </summary>
/// <remarks>
/// <para>
...
/// estado atual e somar/tirar uma zona" fica na camada de orquestracao (ver
/// <c>JflComandoServico</c>), que ja tem acesso a <see cref="CentralStatusQueryService"/> para
/// consultar o estado atual antes de montar o novo conjunto.
/// </para>
/// <para>
...
/// significativo (bit 0) = zona maior. Confirmado com 3 exemplos reais do manual (legado):
...
public sealed class ZoneInhibitCommandService
{
        ...
        string numeroSerie, IReadOnlySet<int> zonasQueDevemFicarInibidas, CancellationToken cancellationToken, TimeSpan? timeout = null)
    {
        ...
        try
        {
            ...
            return ZoneInhibitResult.Ok(status);
        }
